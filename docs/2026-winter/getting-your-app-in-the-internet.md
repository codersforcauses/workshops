...
- [Frinze Lapuz](https://www.linkedin.com/in/frinze-erin-lapuz/), Snr. DevEx Software Engineer at [Atlassian](https://www.atlassian.com/)

![Atlassian](./images/atlassian_logo.svg)

??? info "What is Developer Experience (DevEx)?"
    Developer Experience is a particular specialization in the field of SE that focuses on making developers (human or agents) more productive and efficient.

    The other way to see it is - DevEx is all about building the system that builds the system (primarily software). That system could include:

    - Tools to build the software
    - Culture of innovation in the Engineers that build software
    - (Or) Harnesses for the agents that build software (or as they call it in today's world - Agentic Software Engineering)
...
## (Recommended to follow) A Simple Practical Example of a Server: Your Laptop as a Server for your Phone
...
## (Optional If you want to try...) Assigning a "Local Domain Name" to your laptop
...
## Side Story: Malaysia's Interactive Web App for Chinese New Year 2025 in Kuala Lumpur 

!!! warning "Don't proceed yet until told :)"
    I got some fun stories for you! so hold on!

??? info "Intro"
     As an engineer working remotely, one of the amazing perks really is to travel and see the world in a daily life!

     And of course because of that - back in 2025 I went to Kuala Lumpur it was great!

     <img src="../images/petronas_towers.jpg" style="max-height: 80vh;">

??? info "Exhibition"
    I went to this mall in the Petronas towers called "Suria KLCC". It looked like this!

    <img src="../images/exhibition.jpg" style="max-height: 80vh;">

    One of the things I saw was this interactive web app.

    <!-- Video max height 80vh -->
    <video controls width="100%" style="max-height: 80vh;">
        <source src="../videos/local_web_controller.mp4" type="video/mp4">
        Your browser does not support the video tag. 
        <a href="../videos/local_web_controller.mp4">Download the video</a>.
    </video>
    
   

??? info "Architecture"
    If you take a look at the architecture of the system, it will probably just look like this

    "Push Architecture"

    ```mermaid
    sequenceDiagram
        participant Ipad as Ipad
        participant ServerComputer as Server Computer
        participant DisplayComputer as Display Computer
        
        Ipad->>ServerComputer: Sent the Details about the Wish `/api/wish`
        ServerComputer->>DisplayComputer: (Via Push Notification) Send a request to `/api/display`
        DisplayComputer->>DisplayComputer: Change the display to the new wish
    ```
    ...
    sequenceDiagram
        ...
        Ipad->>ServerComputer: Sent the Details about the Wish `/api/wish`
        ServerComputer->>DataStore: Store the wish in the data store
      
        loop Polling
            DisplayComputer->>ServerComputer: Check if there's a new wish
            ServerComputer->>DataStore: Check if there's a new wish
            DataStore->>ServerComputer: Wish
            ServerComputer->>DisplayComputer: Display the wish
        end
    ```
...
2. Check if it's working using `curl` (a command-line tool that sends a request to a server, just like a browser does):

```bash
curl http://localhost:80
```

You should see the same "Hello World" text response printed directly in your terminal.

3. You can also check it from your own laptop's browser. Go to the browser and type in the IP address of the Droplet, followed by `:80` (the port number) - for example `http://<your-droplet-ip-address>:80`. You should see 
...
Create an "A" record for your subdomain pointed to your server's IP address.

```
my-app.codersforcauses.org. IN A <your-server-ip-address>
```
...
### Getting the files onto your Droplet

Before you can run `docker compose`, you need the `docker-compose.prod.yml` file (and the other files it references, like the nginx config) on your Droplet.

The easiest way is to clone the whole repository onto your Droplet via SSH:

```bash
git clone https://github.com/codersforcauses/django-nextjs-template.git
cd django-nextjs-template
```

??? info "Using your own project instead"
    If you're deploying your own app instead of the example template, clone your own repository instead. Just make sure it has a `docker-compose.prod.yml` (or similar) file that references your own published Docker images.
...
Now edit `.env.prod` and fill in the real values (eg. passwords, secret keys). You can use `nano` (simpler) or `vi`/`vim` (more powerful, usually already installed):

```bash
nano .env.prod
```

??? info "New to vi/vim? Here's a quick cheat sheet"
    `vi`/`vim` behaves differently to most text editors - it has separate "modes" for typing vs giving commands. If you accidentally open it and don't know how to get out, here's what you need:

    ```bash
    vi .env.prod
    ```

    - Press `i` to enter **Insert mode** - now you can type/edit like normal
    - Press `Esc` to leave Insert mode and go back to **Command mode**
    - While in Command mode, type `:wq` then `Enter` to **save and quit**
    - While in Command mode, type `:q!` then `Enter` to **quit without saving** (if you messed up)

    If you're ever stuck, press `Esc` first, then try one of the `:` commands above.
