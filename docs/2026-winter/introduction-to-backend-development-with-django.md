...
??? warning "Common Setup Issues"
    - Make sure Docker Desktop is running before opening the dev container
    - If the dev container fails to build, try rebuilding it from scratch
    - On Windows, ensure WSL2 is enabled for Docker
...
???+ info "Files Created by startapp"
...
??? info "Migration Steps (Text Version)"
    **The migration workflow in 10 steps:**

    1. **Initial State** — You write your model in `models.py` (e.g., `Project: id, name, created_at`). No migrations exist, no database yet.
    2. **`makemigrations`** — Run `python manage.py makemigrations`. Django creates `0001_initial.py` in your `migrations/` folder.
    3. **`migrate` (local)** — Run `python manage.py migrate`. Django applies the migration, creating the table in your local database.
    4. **Deploy to production** — Push your code (including the migration file) to production.
    5. **`migrate` (production)** — Run `python manage.py migrate` in production. The same migration creates the table in the production database.
    6. **Add a field** — You add `content` to your model in `models.py`.
    7. **`makemigrations` again** — Run `python manage.py makemigrations`. Django creates `0002_add_content.py`.
    8. **`migrate` (local) again** — Run `python manage.py migrate`. The new field is added to your local database.
    9. **Deploy to production again** — Push the updated code and new migration file.
    10. **`migrate` (production) again** — Run `python manage.py migrate` in production. The new field is added to the production database.

    **Key takeaway:** Migrations are committed to version control and shared across environments. The database is never modified directly — always through migrations.

??? tip "Pro Tip: Migration Best Practices"
    - **Never edit migration files manually** unless you really know what you're doing
    - **Always commit migration files** to version control
    - **Run migrations in order** — Django tracks which have been applied
...
??? info "How DRF ViewSets Process Requests"
    ```mermaid
    flowchart LR
        ...
        C --> D{Action}
        D -->|GET /items/| E[list]
        D -->|POST /items/| F[create]
        D -->|GET /items/1/| G[retrieve]
        D -->|PUT /items/1/| H[update]
        D -->|DELETE /items/1/| I[destroy]
        E --> J[Serializer]
        F --> J
        G --> J
        H --> J
